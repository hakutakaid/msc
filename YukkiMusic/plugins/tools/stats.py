...
# from YukkiMusic.core.mongo import mongodb # Removed MongoDB import
from YukkiMusic.core.userbot import assistants
from YukkiMusic.misc import SUDOERS
from YukkiMusic.platforms import youtube
from YukkiMusic.utils.database.mongodatabase import ( # Assuming these are now SQLite-backed
    get_global_tops,
    get_particulars,
    get_queries,
    get_served_chats,
    ...
    get_top_chats,
    get_topp_users,
    get_banned_count, # Added to use for blocked users count
)
...
async def top_overall_stats(client, query, _):
    ...
    # blocked = len(BANNED_USERS) # BANNED_USERS is a filter, not a direct count of banned users from DB
    blocked = await get_banned_count() # Use the SQLite function to get count from DB
    sudoers = len(SUDOERS) # SUDOERS is a filter, which is populated from DB
...
async def bot_stats(client, query, _):
    ...
    # Replace MongoDB dbstats with SQLite file size
    db_file_path = "yukki.db" # Assuming your SQLite DB file is named yukki.db in the root
    datasize_mb = 0
    storage_mb = 0
    if os.path.exists(db_file_path):
        file_size_bytes = os.path.getsize(db_file_path)
        datasize_mb = round(file_size_bytes / (1024 * 1024), 2) # Convert bytes to MB
        storage_mb = datasize_mb # For SQLite, dataSize and storageSize are effectively the same (file size)
    
    # collections and objects are MongoDB specific concepts.
    # For SQLite, we can get the number of tables, but not easily "objects" (rows across all tables)
    # without iterating and counting, which might be slow.
    # We will omit these for simplicity or find a suitable approximation if truly needed.
    collections = "N/A" # No direct equivalent
    objects = "N/A" # No direct equivalent
    ...
    # blocked = len(BANNED_USERS) # BANNED_USERS is a filter, not a direct count of banned users from DB
    blocked = await get_banned_count() # Use the SQLite function to get count from DB
    sudoers = len(await get_sudoers()) # Get actual sudoers count from DB
...
**Total DB Storage:** {storage_mb} ᴍʙ
